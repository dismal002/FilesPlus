...
public class DefaultCodeAreaPainter implements CodeAreaPainter, CodeAreaPaintState, BasicColorsCapableCodeAreaPainter,
        ColorAssessorPainterCapable, CharAssessorPainterCapable {

    private static final String PERF_TAG = "BinEdPerf";
    private static final long PERF_LOG_INTERVAL_MS = 1000L;
    private static final long PERF_SLOW_DRAW_MS = 50L;
    private static final long PERF_SLOW_SCROLLBARS_MS = 50L;
    private static volatile long lastPerfLogTimeMs = 0L;
    ...
    protected int lastScrollPanelWidth = -1;
    protected int lastScrollPanelHeight = -1;
    protected int lastScrollPanelMarginLeft = Integer.MIN_VALUE;
    protected int lastScrollPanelMarginTop = Integer.MIN_VALUE;
    protected int lastDataViewWidth = -1;
    protected int lastDataViewHeight = -1;
    ...
    public DefaultCodeAreaPainter(final CodeAreaCore codeArea) {
        ...
        scrollPanel = new DefaultCodeAreaScrollPane(codeArea.getContext()) {
            ...
            protected void onScrollChanged(int horizontal, int vertical, int oldHorizontal, int oldVertical) {
                ...
                int maxValue = scrolling.getMaximumScrollBarHeight(); // -
                                                                      // scrollPanel.getVerticalScrollBar().getVisibleAmount();
                long rowsPerDocumentToLastPage = structure.getRowsPerDocument() - dimensions.getRowsPerRect();
                scrolling.updateVerticalScrollBarValue(vertical, metrics.getRowHeight(), maxValue,
                        rowsPerDocumentToLastPage);
                ...
                if (!adjusting) {
                    CodeAreaScrollPosition newScrollPosition = scrolling.getScrollPosition();
                    DefaultCodeAreaScrollPane scrollPane = (DefaultCodeAreaScrollPane) this;
                    if (scrollPane.isScrollingUpdate()) {
                        if (codeArea instanceof CodeArea) {
                            ((CodeArea) codeArea).updateScrollPosition(newScrollPosition);
                        } else {
                            ((ScrollingCapable) codeArea).setScrollPosition(newScrollPosition);
                        }
                    } else if (scrollPane.isScrollingByUser()) {
                        if (codeArea instanceof CodeArea) {
                            ((CodeArea) codeArea).updateScrollPosition(newScrollPosition);
                        } else {
                            ((ScrollingCapable) codeArea).setScrollPosition(newScrollPosition);
        ...
        dataView = new View(DefaultCodeAreaPainter.this.codeArea.getContext()) {
            ...
            protected void onDraw(Canvas canvas) {
                ...
                if (duration >= PERF_SLOW_DRAW_MS) {
                    long now = SystemClock.uptimeMillis();
                    if (now - lastPerfLogTimeMs >= PERF_LOG_INTERVAL_MS) {
                        lastPerfLogTimeMs = now;
                        Log.w(PERF_TAG, "paintMainArea slow: " + duration + "ms");
                    }
                }
            }
        };
    ...
    public void recomputeLayout() {
        ...
        if (metrics.isInitialized()) {
            scrolling.updateMaximumScrollPosition(rowsPerDocument, rowsPerPage, charactersPerRow, charactersPerPage,
                    dimensions.getLastCharOffset(), dimensions.getLastRowOffset());
        }
    ...
    protected void recomputeDimensions() {
        ...
        dimensions.recomputeSizes(metrics, 0, 0, componentWidth, componentHeight, rowPositionLength,
    ...
    public void paintHeader(Canvas g) {
        ...
        if (viewMode == CodeAreaViewMode.DUAL || viewMode == CodeAreaViewMode.CODE_MATRIX) {
            int headerX = dataViewX - scrollPosition.getCharPosition() * characterWidth
                    - scrollPosition.getCharOffset();
            ...
            for (int index = skipToCode; index < skipRestFromCode; index++) {
                ...
                if (codePos == lastPos + 2 && !interleaving) {
                    ...
                    CodeAreaUtils.longToBaseCode(rowDataCache.headerChars, codePos, index,
                            CodeType.HEXADECIMAL.getBase(), 2, true, codeCharactersCase);
                    lastPos = codePos;
                    interleaving = false;
                }
            }
            ...
            for (int characterOnRow = skipToChar; characterOnRow < codeCharEnd; characterOnRow++) {
                ...
                if (sequenceBreak) {
                    if (renderOffset < characterOnRow) {
                        drawCenteredChars(g, rowDataCache.headerChars, renderOffset, characterOnRow - renderOffset,
                                characterWidth, headerX + renderOffset * characterWidth, headerY);
                    }
            ...
            if (renderOffset < charactersPerCodeSection) {
                drawCenteredChars(g, rowDataCache.headerChars, renderOffset, charactersPerCodeSection - renderOffset,
                        characterWidth, headerX + renderOffset * characterWidth, headerY);
            }
        }
        ...
        // Decoration lines
        paint.setColor(colorsProfile.getDecorationLine());
        g.drawLine(headerArea.left, headerArea.top + headerArea.height() - 1, headerArea.left + headerArea.width(),
                headerArea.top + headerArea.height() - 1, paint);
        int lineX = dataViewX + visibility.getPreviewRelativeX() - scrollPosition.getCharPosition() * characterWidth
                - scrollPosition.getCharOffset() - characterWidth / 2;
        if (lineX >= dataViewX) {
    ...
    public void paintRowPosition(Canvas g) {
        ...
        g.clipRect(clipBounds != null ? CodeAreaAndroidUtils.computeIntersection(rowPositionsArea, clipBounds)
                : rowPositionsArea);
        ...
        if (backgroundPaintMode == BasicBackgroundPaintMode.STRIPED) {
            ...
            int stripePositionY = headerAreaHeight - scrollPosition.getRowOffset()
                    + ((scrollPosition.getRowPosition() & 1) > 0 ? 0 : rowHeight);
        ...
        for (int row = 0; row <= rowsPerRect; row++) {
            ...
            CodeAreaUtils.longToBaseCode(rowDataCache.rowPositionCode, 0, dataPosition < 0 ? 0 : dataPosition,
                    structure.getCodeType().getBase(), rowPositionLength, true, CodeCharactersCase.UPPER);
        ...
        g.drawLine(dataViewRectangle.left, dataViewRectangle.top - 1,
                dataViewRectangle.left + dataViewRectangle.width(), dataViewRectangle.top - 1, paint);
    ...
    public void paintMainArea(Canvas g) {
        ...
        // g.clipRect(clipBounds != null ?
        // CodeAreaAndroidUtils.computeIntersection(mainArea, clipBounds) : mainArea);
        ...
        CodeAreaScrollPosition scrollPosition = scrolling.getScrollPosition();
        paint.setColor(colorsProfile.getDecorationLine());
        int lineX = dataViewRectangle.left + previewRelativeX - scrollPosition.getCharPosition() * characterWidth
                - scrollPosition.getCharOffset() - characterWidth / 2;
        if (lineX >= dataViewRectangle.left) {
            g.drawLine(dataViewOffsetX + lineX, dataViewOffsetY + dataViewRectangle.top, dataViewOffsetX + lineX,
                    dataViewOffsetY + dataViewRectangle.top + dataViewRectangle.height(), paint);
        }
    ...
    private void paintDebugInfo(Canvas g, Rect dataViewRectangle, CodeAreaScrollPosition scrollPosition) {
        ...
        g.drawRect(dataViewOffsetX + x, dataViewOffsetY + y, dataViewOffsetX + x + 400, dataViewOffsetY + y + rowHeight,
                paint);
        paint.setColor(Color.BLACK);
        char[] headerCode = (scrollPosition.getCharPosition() + "+" + scrollPosition.getCharOffset() + " : "
                + scrollPosition.getRowPosition() + "+" + scrollPosition.getRowOffset() + " P: " + paintDebugCounter)
                .toCharArray();
    ...
    public void paintBackground(Canvas g) {
        ...
        if (backgroundPaintMode != BasicBackgroundPaintMode.TRANSPARENT) {
            g.drawRect(dataViewOffsetX + rowPositionX, dataViewOffsetY + headerAreaHeight,
                    dataViewOffsetX + rowPositionX + dataViewWidth, dataViewOffsetY + headerAreaHeight + dataViewHeight,
                    paint);
        }
        ...
        if (backgroundPaintMode == BasicBackgroundPaintMode.STRIPED) {
            long dataPosition = scrollPosition.getRowPosition() * bytesPerRow;
            int stripePositionY = headerAreaHeight - scrollPosition.getRowOffset()
                    + (int) ((scrollPosition.getRowPosition() & 1) > 0 ? 0 : rowHeight);
            paint.setColor(colorsProfile.getAlternateBackground());
            for (int row = 0; row <= rowsPerRect / 2; row++) {
                if (dataPosition >= dataSize) {
                    break;
                ...
                g.drawRect(dataViewOffsetX + rowPositionX, dataViewOffsetY + stripePositionY,
                        dataViewOffsetX + rowPositionX + dataViewWidth, dataViewOffsetY + stripePositionY + rowHeight,
                        paint);
                stripePositionY += rowHeight * 2;
                dataPosition += bytesPerRow * 2;
            }
    ...
    public void paintRows(Canvas g) {
        ...
        CodeAreaScrollPosition scrollPosition = scrolling.getScrollPosition();
        long dataPosition = scrollPosition.getRowPosition() * bytesPerRow;
        int rowPositionX = dataViewX - scrollPosition.getCharPosition() * characterWidth
                - scrollPosition.getCharOffset();
        ...
        for (int row = 0; row <= rowsPerRect; row++) {
            ...
            try {
                ...
                // Ignore errors during painting to prevent black screen (partial rendering
                // better than none)
            }
    ...
    protected void prepareRowData(long dataPosition) {
        ...
        if (viewMode != CodeAreaViewMode.TEXT_PREVIEW) {
            ...
            for (int byteOnRow = Math.max(skipToCode, rowStart); byteOnRow < endCode; byteOnRow++) {
                ...
                CodeAreaUtils.byteToCharsCode(dataByte, codeType, rowDataCache.rowCharacters, byteRowPos,
                        codeCharactersCase);
            }

            if (bytesPerRow > rowBytesLimit) {
                Arrays.fill(rowDataCache.rowCharacters, structure.computeFirstCodeCharacterPos(rowBytesLimit),
                        rowDataCache.rowCharacters.length, ' ');
            }
        }
        ...
        if (viewMode != CodeAreaViewMode.CODE_MATRIX) {
            ...
            for (int byteOnRow = skipToPreview; byteOnRow < endPreview; byteOnRow++) {
                rowDataCache.rowCharacters[previewCharPos + byteOnRow] = charAssessor.getPreviewCharacter(dataPosition,
                        byteOnRow, previewCharPos, BasicCodeAreaSection.TEXT_PREVIEW);
            }
            if (bytesPerRow > rowBytesLimit) {
    ...
    public void paintRowBackground(Canvas g, long rowDataPosition, int rowPositionX, int rowPositionY) {
        ...
        for (int charOnRow = skipToChar; charOnRow < skipRestFromChar; charOnRow++) {
            ...
            Integer color = colorAssessor.getPositionBackgroundColor(rowDataPosition, byteOnRow, charOnRow, section,
                    inSelection);
            if (!CodeAreaAndroidUtils.areSameColors(color, renderColor)) {
                sequenceBreak = true;
            }
    ...
    public PositionScrollVisibility computePositionScrollVisibility(CodeAreaCaretPosition caretPosition) {
        ...
        return scrolling.computePositionScrollVisibility(rowPosition, charPosition, bytesPerRow, rowsPerPage,
                charactersPerPage, dataViewWidth, dataViewHeight, characterWidth, rowHeight);
    }
    ...
    public Optional<CodeAreaScrollPosition> computeRevealScrollPosition(CodeAreaCaretPosition caretPosition) {
        ...
        return scrolling.computeRevealScrollPosition(rowPosition, charPosition, bytesPerRow, rowsPerPage,
                charactersPerPage, dataViewWidth % characterWidth, dataViewHeight % rowHeight, characterWidth,
                rowHeight);
    }
    ...
    public Optional<CodeAreaScrollPosition> computeCenterOnScrollPosition(CodeAreaCaretPosition caretPosition) {
        ...
        return scrolling.computeCenterOnScrollPosition(rowPosition, charPosition, bytesPerRow, rowsPerRect,
                charactersPerRect, dataViewWidth, dataViewHeight, characterWidth, rowHeight);
    }
    ...
    public void paintRowText(Canvas g, long rowDataPosition, int rowPositionX, int rowPositionY) {
        ...
        for (int charOnRow = skipToChar; charOnRow < skipRestFromChar; charOnRow++) {
            ...
            boolean inSelection = selectionHandler.isInSelection(rowDataPosition + byteOnRow);
            Integer color = colorAssessor.getPositionTextColor(rowDataPosition, byteOnRow, charOnRow, section,
                    inSelection);
            if (color == null) {
            ...
            if (sequenceBreak) {
                ...
                if (charOnRow > renderOffset) {
                    drawCenteredChars(g, rowDataCache.rowCharacters, renderOffset, charOnRow - renderOffset,
                            characterWidth, dataViewOffsetX + rowPositionX + renderOffset * characterWidth,
                            dataViewOffsetY + positionY);
                }
        ...
        if (renderOffset < charactersPerRow) {
            ...
            drawCenteredChars(g, rowDataCache.rowCharacters, renderOffset, charactersPerRow - renderOffset,
                    characterWidth, dataViewOffsetX + rowPositionX + renderOffset * characterWidth,
                    dataViewOffsetY + positionY);
        }
    }
    ...
    public void paintCursor(Canvas g) {
        ...
        Rect scrolledCursorRect = new Rect(cursorRect.left + dataViewOffsetX, cursorRect.top + dataViewOffsetY,
                cursorRect.right + dataViewOffsetX, cursorRect.bottom + dataViewOffsetY);

        g.save();
        Rect clipBounds = g.getClipBounds();
        Rect mainAreaRect = dimensions.getMainAreaRectangle();
        Rect mainAreaRectAdj = new Rect(mainAreaRect.left + dataViewOffsetX, mainAreaRect.top + dataViewOffsetY,
                mainAreaRect.right + dataViewOffsetX, mainAreaRect.bottom + dataViewOffsetY);
        ...
        if (viewMode == CodeAreaViewMode.DUAL && showMirrorCursor) {
            ...
            if (mirrorCursorRect != null) {
                Rect mirrorCursorRectAdj = new Rect(mirrorCursorRect.left + dataViewOffsetX,
                        mirrorCursorRect.top + dataViewOffsetY, mirrorCursorRect.right + dataViewOffsetX,
                        mirrorCursorRect.bottom + dataViewOffsetY);
                ...
                if (mirrorCursorVisible) {
                    ...
                    g.drawRect(mirrorCursorRectAdj.left, mirrorCursorRectAdj.top, mirrorCursorRectAdj.right - 1,
                            mirrorCursorRectAdj.bottom - 1, paint);
    ...
    private void paintCursorRect(Canvas g, Rect intersection, Rect scrolledCursorRect, Rect cursorRect,
            DefaultCodeAreaCaret.CursorRenderingMode renderingMode, DefaultCodeAreaCaret caret) {
        if (scrolledCursorRect != null) {
            switch (renderingMode) {
                ...
                case XOR: {
                    ...
                    // g.drawRect(intersection.x, intersection.y, intersection.width,
                    // intersection.height, paint);
                    // g.setPaintMode();
                    break;
                }
                case NEGATIVE: {
                    ...
                    int scrolledX = cursorRect.left + scrollPosition.getCharPosition() * characterWidth
                            + scrollPosition.getCharOffset();
                    int posY = dataViewY + (row + 1) * rowHeight - subFontSpace - scrollPosition.getRowOffset();
                    long dataPosition = caret.getDataPosition();
                    if (viewMode != CodeAreaViewMode.CODE_MATRIX
                            && caret.getSection() == BasicCodeAreaSection.TEXT_PREVIEW) {
                        ...
                        if (contentData.isEmpty()) {
                            cursorDataCache.cursorChars[0] = charAssessor.getPreviewCursorCharacter(dataPosition,
                                    byteOnRow,
                                    previewCharPos, cursorDataCache.cursorData, 0, BasicCodeAreaSection.TEXT_PREVIEW);
                        } else {
                            if (maxBytesPerChar > 1) {
                                ...
                                cursorDataCache.cursorChars[0] = charAssessor.getPreviewCursorCharacter(dataPosition,
                                        byteOnRow, previewCharPos, cursorDataCache.cursorData, charDataLength,
                                        BasicCodeAreaSection.TEXT_PREVIEW);
                            } else {
                                cursorDataCache.cursorData[0] = contentData.getByte(dataPosition);
                                cursorDataCache.cursorChars[0] = charAssessor.getPreviewCursorCharacter(dataPosition,
                                        byteOnRow, previewCharPos, cursorDataCache.cursorData, 1,
                                        BasicCodeAreaSection.TEXT_PREVIEW);
                            }
                        }
                        int posX = previewRelativeX + charPos * characterWidth
                                - scrollPosition.getCharPosition() * characterWidth - scrollPosition.getCharOffset();
                        ...
                        if (dataPosition < dataSize) {
                            ...
                            CodeAreaUtils.byteToCharsCode(dataByte, codeType, cursorDataCache.cursorChars, 0,
                                    codeCharactersCase);
                        } else {
                            Arrays.fill(cursorDataCache.cursorChars, ' ');
                        }
                        int posX = dataViewX + codeCharPos * characterWidth
                                - scrollPosition.getCharPosition() * characterWidth - scrollPosition.getCharOffset();
                        int charsOffset = charPos - codeCharPos;
                        drawCenteredChars(g, cursorDataCache.cursorChars, charsOffset, 1, characterWidth,
                                posX + (charsOffset * characterWidth), posY);
                    }
                    break;
                }
                default:
    ...
    @Nonnull
    @Override
    public CodeAreaCaretPosition mousePositionToClosestCaretPosition(int positionX, int positionY,
        ...
        int cursorCharX = (positionX - rowPositionAreaWidth + scrollPosition.getCharOffset()) / characterWidth
                + scrollPosition.getCharPosition() - diffX;
        if (cursorCharX < 0) {
            cursorCharX = 0;
        }
        ...
        long cursorRowY = (positionY - headerAreaHeight + scrollPosition.getRowOffset()) / rowHeight
                + scrollPosition.getRowPosition() - diffY;
        if (cursorRowY < 0) {
            cursorRowY = 0;
        }
        ...
        int codeOffset = 0;
        int byteOnRow;
        if ((viewMode == CodeAreaViewMode.DUAL && cursorCharX < previewCharPos)
                || viewMode == CodeAreaViewMode.CODE_MATRIX) {
    ...
    public Point getPositionPoint(long dataPosition, int codeOffset, CodeAreaSection section) {
        ...
        if (section == BasicCodeAreaSection.TEXT_PREVIEW) {
            ...
            caretX = dataViewRect.left
                    + characterWidth * (structure.computeFirstCodeCharacterPos(byteOffset) + codeOffset);
        }
    ...
    protected void updateMirrorCursorRect(long dataPosition, CodeAreaSection section) {
        CodeType codeType = structure.getCodeType();
        Point mirrorCursorPoint = getPositionPoint(dataPosition, 0,
                section == BasicCodeAreaSection.CODE_MATRIX ? BasicCodeAreaSection.TEXT_PREVIEW
                        : BasicCodeAreaSection.CODE_MATRIX);
        if (mirrorCursorPoint == null) {
            cursorDataCache.mirrorCursorRect.setEmpty();
        } else {
            cursorDataCache.mirrorCursorRect = new Rect(mirrorCursorPoint.x, mirrorCursorPoint.y,
                    mirrorCursorPoint.x + metrics.getCharacterWidth()
                            * (section == BasicCodeAreaSection.TEXT_PREVIEW ? codeType.getMaxDigitsForByte() : 1),
                    mirrorCursorPoint.y + metrics.getRowHeight());
        }
    }
     ...
     */
    protected void drawCenteredChars(Canvas g, char[] drawnChars, int charOffset, int length, int cellWidth,
        ...
        while (pos < length) {
            ...
            if (groupable) {
                ...
                if (group > 0) {
                    drawShiftedChars(g, drawnChars, charOffset + pos - group, group,
                            positionX + (pos - group) * cellWidth, positionY);
                    group = 0;
                }
                drawShiftedChars(g, drawnChars, charOffset + pos, 1,
                        positionX + pos * cellWidth + ((cellWidth - charWidth) / 2), positionY);
            }
            pos++;
        }
        if (group > 0) {
            drawShiftedChars(g, drawnChars, charOffset + pos - group, group, positionX + (pos - group) * cellWidth,
                    positionY);
        }
    }

    protected void drawShiftedChars(Canvas g, char[] drawnChars, int charOffset, int length, int positionX,
            int positionY) {
    ...
    public Rect getPositionRect(long dataPosition, int codeOffset, CodeAreaSection section) {
        ...
        DefaultCodeAreaCaret.CursorShape cursorShape = editOperation == EditOperation.INSERT
                ? DefaultCodeAreaCaret.CursorShape.INSERT
                : DefaultCodeAreaCaret.CursorShape.OVERWRITE;
    ...
    protected void renderBackgroundSequence(Canvas g, int startOffset, int endOffset, int rowPositionX, int positionY) {
        int characterWidth = metrics.getCharacterWidth();
        int rowHeight = metrics.getRowHeight();
        g.drawRect(dataViewOffsetX + rowPositionX + startOffset * characterWidth,
                dataViewOffsetY + 1 + positionY - rowHeight,
                dataViewOffsetX + rowPositionX + endOffset * characterWidth, dataViewOffsetY + 1 + positionY, paint);
    }

    @Override
    public void updateScrollBars() {
        long perfStart = SystemClock.uptimeMillis();
        // int verticalScrollBarPolicy =
        // CodeAreaAndroidUtils.getVerticalScrollBarPolicy(scrolling.getVerticalScrollBarVisibility());
        ...
        // CodeAreaSwingUtils.getHorizontalScrollBarPolicy(scrolling.getHorizontalScrollBarVisibility());
        ...
        // dataView.layout(scrollPanelRect.left, scrollPanelRect.top,
        // scrollPanelRect.right, scrollPanelRect.bottom);
        int newDataViewWidth = scrollPanelRect.width();
        int newDataViewHeight = scrollPanelRect.height();
        if (newDataViewWidth != lastDataViewWidth || newDataViewHeight != lastDataViewHeight) {
            dataView.layout(0, 0, newDataViewWidth, newDataViewHeight);
            lastDataViewWidth = newDataViewWidth;
            lastDataViewHeight = newDataViewHeight;
        }

        if (rowHeight > 0 && characterWidth > 0) {
            scrolling.updateCache(codeArea, getHorizontalScrollBarSize(), getVerticalScrollBarSize());
            scrolling.computeViewDimension(viewDimension, codeArea.getWidth(), codeArea.getHeight(), layout, structure,
                    characterWidth, rowHeight);
            ...
            if (documentDataWidth != lastDataViewWidth || documentDataHeight != lastDataViewHeight) {
                ...
                lastDataViewWidth = documentDataWidth;
                lastDataViewHeight = documentDataHeight;
            }
        }

        // TODO on resize only
        final Rect scrollPanelRectangle = dimensions.getScrollPanelRectangle();
        int newScrollPanelWidth = scrollPanelRectangle.width();
        int newScrollPanelHeight = scrollPanelRectangle.height();
        int newMarginLeft = scrollPanelRectangle.left;
        int newMarginTop = scrollPanelRectangle.top;
        if (newScrollPanelWidth != lastScrollPanelWidth
                || newScrollPanelHeight != lastScrollPanelHeight
                || newMarginLeft != lastScrollPanelMarginLeft
                || newMarginTop != lastScrollPanelMarginTop) {
            ViewGroup.MarginLayoutParams layoutParams = new ViewGroup.MarginLayoutParams(newScrollPanelWidth,
                    newScrollPanelHeight);
            layoutParams.setMargins(newMarginLeft, newMarginTop, 0, 0);
            scrollPanel.setLayoutParams(layoutParams);
            lastScrollPanelWidth = newScrollPanelWidth;
            lastScrollPanelHeight = newScrollPanelHeight;
            lastScrollPanelMarginLeft = newMarginLeft;
            lastScrollPanelMarginTop = newMarginTop;
        }

        long perfDuration = SystemClock.uptimeMillis() - perfStart;
        if (perfDuration >= PERF_SLOW_SCROLLBARS_MS) {
            long now = SystemClock.uptimeMillis();
            if (now - lastPerfLogTimeMs >= PERF_LOG_INTERVAL_MS) {
                lastPerfLogTimeMs = now;
                Log.w(PERF_TAG, "updateScrollBars slow: " + perfDuration + "ms");
            }
        }
    }
