...
public class CodeAreaOperationCommandHandler implements CodeAreaCommandHandler {
    ...
    public CodeAreaOperationCommandHandler(Context context, CodeAreaCore codeArea, BinaryDataUndoRedo undoRedo) {
         ...
         * updateCanPaste();
         * });
         * binedDataFlavor = new DataFlavor(BinaryData.class,
         * DefaultCodeAreaCommandHandler.BINED_CLIPBOARD_MIME_FULL);
         * try {
         * binaryDataFlavor = new DataFlavor(CodeAreaUtils.MIME_CLIPBOARD_BINARY);
         * } catch (ClassNotFoundException ex) {
         * Logger.getLogger(CodeAreaOperationCommandHandler.class.getName()).log(Level.
         * SEVERE, null, ex);
         * }
         * updateCanPaste();
    ...
    public static CodeAreaCommandHandler.CodeAreaCommandHandlerFactory createDefaultCodeAreaCommandHandlerFactory(
        ...
        return (CodeAreaCore codeAreaCore) -> new CodeAreaOperationCommandHandler(context, codeAreaCore,
                new CodeAreaUndoRedo(codeAreaCore));
    }
    ...
    private void updateCanPaste() {
        canPaste = clipboard.hasPrimaryClip(); // canPaste = CodeAreaSwingUtils.canPaste(clipboard, binedDataFlavor) ||
                                               // CodeAreaSwingUtils.canPaste(clipboard, DataFlavor.stringFlavor);
    }
            ...
            default: {
                if (((ClipboardCapable) codeArea).getClipboardHandlingMode() == ClipboardHandlingMode.PROCESS) {
                    if ((keyEvent.getModifiers() & metaMask) > 0 && keyEvent.getKeyCode() == KeyEvent.KEYCODE_C) {
                        ...
                        // keyEvent.consume();
                        break;
                    } else if ((keyEvent.getModifiers() & metaMask) > 0
                            && keyEvent.getKeyCode() == KeyEvent.KEYCODE_X) {
                        cut();
                        // keyEvent.consume();
                        break;
                    } else if ((keyEvent.getModifiers() & metaMask) > 0
                            && keyEvent.getKeyCode() == KeyEvent.KEYCODE_V) {
                        paste();
                        // keyEvent.consume();
                        break;
                    } else if ((keyEvent.getModifiers() & metaMask) > 0
                            && keyEvent.getKeyCode() == KeyEvent.KEYCODE_A) {
                        codeArea.selectAll();
                        // keyEvent.consume();
                        break;
                    }
    ...
    public void keyTyped(int keyCode, KeyEvent keyEvent) {
        ...
        android.util.Log.d("BinEd-Input", "keyTyped: " + keyValue + " (code: " + keyCode + ")");
        // TODO Add support for high unicode codes
        if (keyValue == KeyEvent.KEYCODE_UNKNOWN) {
            android.util.Log.d("BinEd-Input", "keyTyped: Unknown key code, skipping");
            return;
        }
        if (!checkEditAllowed()) {
            android.util.Log.d("BinEd-Input", "keyTyped: Edit not allowed, skipping");
            return;
        }

        CodeAreaSection section = ((CaretCapable) codeArea).getActiveSection();
        android.util.Log.d("BinEd-Input", "keyTyped: Section " + section.toString());
        if (section != BasicCodeAreaSection.TEXT_PREVIEW) {
            ...
            if (keyValue > DefaultCodeAreaCommandHandler.LAST_CONTROL_CODE && keyValue != DELETE_CHAR) {
                ...
                android.util.Log.d("BinEd-Input", "keyTyped: Control code or delete in preview, skipping");
            }
        }
    }
    ...
    private void pressedCharAsCode(char keyChar) {
        ...
        if (validKey) {
            ...
            if (editMode == EditMode.INPLACE) {
                EditCodeDataCommand command = new EditCodeDataCommand(codeArea,
                        EditDataCommand.EditOperationType.OVERWRITE, dataPosition, codeOffset, (byte) value);
                ...
                if (editOperation == EditOperation.OVERWRITE) {
                    if (deleteSelectionCommand != null) {
                        ...
                        editCommand = new EditCodeDataCommand(codeArea, EditDataCommand.EditOperationType.OVERWRITE,
                                dataPosition, codeOffset, (byte) value);
                        compoundCommand.addCommand(editCommand);
                        undoRedo.execute(compoundCommand);
                    } else {
                        EditCodeDataCommand command = new EditCodeDataCommand(codeArea,
                                EditDataCommand.EditOperationType.OVERWRITE, dataPosition, codeOffset, (byte) value);
                        if (editCommand != null && isAppendAllowed()
                                && undoRedo instanceof BinaryDataAppendableUndoRedo) {
                            if (!((BinaryDataAppendableUndoRedo) undoRedo).appendExecute(command)) {
                                editCommand = command;
                            }
                        } else {
                    ...
                    if (deleteSelectionCommand != null) {
                        CodeAreaCompoundCommand compoundCommand = new CodeAreaCompoundCommand(codeArea);
                        compoundCommand.addCommand(deleteSelectionCommand);
                        editCommand = new EditCodeDataCommand(codeArea, EditDataCommand.EditOperationType.INSERT,
                                dataPosition, codeOffset, (byte) value);
                        compoundCommand.addCommand(editCommand);
                        undoRedo.execute(compoundCommand);
                    } else {
                        EditCodeDataCommand command = new EditCodeDataCommand(codeArea,
                                EditDataCommand.EditOperationType.INSERT, dataPosition, codeOffset, (byte) value);
                        if (editCommand != null && isAppendAllowed()
                                && undoRedo instanceof BinaryDataAppendableUndoRedo) {
                            if (!((BinaryDataAppendableUndoRedo) undoRedo).appendExecute(command)) {
                                editCommand = command;
    ...
    private void pressedCharInPreview(char keyChar) {
        if (isValidChar(keyChar)) {
            ...
            long dataPosition = ((CaretCapable) codeArea).getDataPosition();
            if (editMode == EditMode.INPLACE) {
                EditCharDataCommand command = new EditCharDataCommand(codeArea,
                        EditDataCommand.EditOperationType.OVERWRITE, dataPosition, keyChar);
                if (editCommand != null && isAppendAllowed() && undoRedo instanceof BinaryDataAppendableUndoRedo) {
                    if (!((BinaryDataAppendableUndoRedo) undoRedo).appendExecute(command)) {
                        editCommand = command;
                    }
                ...
                if (editOperation == EditOperation.OVERWRITE) {
                    if (deleteCommand != null) {
                        ...
                        editCommand = new EditCharDataCommand(codeArea, EditDataCommand.EditOperationType.OVERWRITE,
                                dataPosition, keyChar);
                        compoundCommand.addCommand(editCommand);
                        undoRedo.execute(compoundCommand);
                    } else {
                        EditCharDataCommand command = new EditCharDataCommand(codeArea,
                    ...
                    if (deleteCommand != null) {
                        CodeAreaCompoundCommand compoundCommand = new CodeAreaCompoundCommand(codeArea);
                        compoundCommand.addCommand(deleteCommand);
                        editCommand = new EditCharDataCommand(codeArea, EditDataCommand.EditOperationType.INSERT,
                                dataPosition, keyChar);
                        compoundCommand.addCommand(editCommand);
                        undoRedo.execute(compoundCommand);
                    } else {
                        EditCharDataCommand command = new EditCharDataCommand(codeArea,
                                EditDataCommand.EditOperationType.INSERT, dataPosition, keyChar);
                        if (editCommand != null && isAppendAllowed()
                                && undoRedo instanceof BinaryDataAppendableUndoRedo) {
                            if (!((BinaryDataAppendableUndoRedo) undoRedo).appendExecute(command)) {
                                editCommand = command;
    ...
    public void tabPressed(SelectingMode selectingMode) {
        ...
        if (tabKeyHandlingMode == TabKeyHandlingMode.PLATFORM_SPECIFIC
                || tabKeyHandlingMode == TabKeyHandlingMode.CYCLE_TO_NEXT_SECTION
                || tabKeyHandlingMode == TabKeyHandlingMode.CYCLE_TO_PREVIOUS_SECTION) {
    ...
    protected void deleteAction(char keyChar) {
        if (codeArea.hasSelection()) {
            ...
            if (section == BasicCodeAreaSection.CODE_MATRIX) {
                EditCodeDataCommand command = new EditCodeDataCommand(codeArea,
                        EditDataCommand.EditOperationType.DELETE, dataPosition, 0, (byte) keyChar);
                if (editCommand != null && isAppendAllowed() && undoRedo instanceof BinaryDataAppendableUndoRedo) {
                    if (!((BinaryDataAppendableUndoRedo) undoRedo).appendExecute(command)) {
                        editCommand = command;
                    }
                ...
                EditCharDataCommand command = new EditCharDataCommand(codeArea,
                        EditDataCommand.EditOperationType.DELETE, dataPosition, keyChar);
                if (editCommand != null && isAppendAllowed() && undoRedo instanceof BinaryDataAppendableUndoRedo) {
                    if (!((BinaryDataAppendableUndoRedo) undoRedo).appendExecute(command)) {
                        editCommand = command;
                    }
    ...
    public void copy() {
        ...
        if (!selection.isEmpty()) {
            ...
            ClipData clipData = CodeAreaAndroidUtils.createBinaryDataClipboardData(context, copy, binedDataFlavor,
                    binaryDataFlavor, charset);
    ...
    public void copyAsCode() {
        ...
        if (!selection.isEmpty()) {
            ...
            ClipData clipData = CodeAreaAndroidUtils.createCodeDataClipboardData(context, copy, binedDataFlavor,
                    codeType, charactersCase);
            setClipboardContent(clipData);
        }
    }
        ...
        try {
            ...
            try {
                if (!description.hasMimeType(CodeAreaUtils.BINED_CLIPBOARD_MIME)
                        && !description.hasMimeType(CodeAreaUtils.MIME_CLIPBOARD_BINARY)
            ...
            if (description.hasMimeType(CodeAreaUtils.BINED_CLIPBOARD_MIME)) {
                ...
                // InputStream clipboardData = (InputStream)
                // clipboard.getData(binaryDataFlavor);
                ...
                if (clipboardData != null) {
                    byte[] bytes = clipboardData.toString()
                            .getBytes(Charset.forName(CodeAreaAndroidUtils.DEFAULT_ENCODING));
    ...
    private void pasteBinaryData(BinaryData pastedData) {
        ...
        CodeAreaCommand pasteCommand = CodeAreaCompoundCommand.buildCompoundCommand(codeArea, deleteSelectionCommand,
                pasteDataCommand);
    ...
    public void moveCaret(int positionX, int positionY, SelectingMode selecting) {
        CodeAreaCaretPosition caretPosition = ((CaretCapable) codeArea).mousePositionToClosestCaretPosition(positionX,
                positionY, CaretOverlapMode.PARTIAL_OVERLAP);
    ...
    public void scroll(ScrollingDirection direction) {
        ...
        CodeAreaScrollPosition scrollPosition = ((ScrollingCapable) codeArea).computeScrolling(sourcePosition,
                direction);
